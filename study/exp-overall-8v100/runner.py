...
cfg_list_collector.concat(cur_common_base.copy().override('dataset', [Dataset.friendster,       ]).override('cache_percent', percent_gen( 5, 30, 5)))
# # 1.2 unsup, mag 240 requires different batch
cfg_list_collector.concat(cur_common_base.copy().override('dataset', [Dataset.mag240m_homo,     ])
                                                .override('cache_percent', [0.01] + percent_gen( 2, 6, 2) + [0.07, 0.08])
                                                .override('batchsize', [2000, 1000]))
...
cfg_list_collector.concat(cur_common_base.copy().override('dataset', [Dataset.mag240m_homo,     ])
                                                .override('cache_percent', [0.01] + percent_gen( 2, 6, 2))
                                                .override('batchsize', [8000, 4000]))
...
cfg_list_collector.hyper_override(
  ...
  [
    ...
    [False, CachePolicy.partition, "", ""]
])

cfg_list_collector.override('coll_cache_scale', [0, 16])
cfg_list_collector.override('use_amp', [True, False])
